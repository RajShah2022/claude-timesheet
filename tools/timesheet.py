...
Output:
    1. Per-day breakdown: all user messages you typed + key agent task descriptions
    2. Monthly summary grouped by project + theme

Main session files (depth-1 under project dir) contain all messages you typed.
...
def parse_time_range(time_str):
    ...
    if not time_str:
        ...
        end   = datetime(now.year + (now.month == 12), (now.month % 12) + 1, 1, 0, 0, 0, tzinfo=local_tz)
...
def main():
    ap = argparse.ArgumentParser(description="Timesheet from Claude Code sessions")
    ap.add_argument("--month", default="", help="Month or exact date to summarise (e.g. 'Feb 2026', '2026-02', '12 Feb 2026', '2026-02-12'). Defaults to current month.")
    ...
    print(f"Range: {month_label} | UTC: {after_utc.strftime('%Y-%m-%dT%H:%M:%SZ')} → {before_utc.strftime('%Y-%m-%dT%H:%M:%SZ')}", flush=True)
    ...
    #   label "user"  = messages you typed (from main session JSONL files)
    ...
    # Stage 2a: ALL user messages from main session files (direct children of project dir).
    # message.role == "user" with plain-text content = something you typed.
    # Turns where content is only tool_result/tool_use blocks are skipped (Claude internals).
    user_msg_count = 0
        ...
        try:
            with open(f, "r", errors="replace") as fh:
                for line in fh:
                    ...
                    # Skip slash commands (e.g. /compact, /plan)
                    ...
                    if text:
                        ...
                        maybe_add(ts.timestamp(), time_str, "user", proj, text)
                        user_msg_count += 1
        except Exception:
            pass
    print(f"User messages from session files: {user_msg_count}", flush=True)

    # Stage 2b: sub-agent task descriptions and first assistant responses from all session files.
    # Main files: collect events for hours/themes + first assistant response per session.
    # Agent files: also grab first user message (the task injected into the sub-agent).
    for is_agent, proj, f in [(False, p, fi) for p, fi in main_files] + [(True, p, fi) for p, fi in agent_files]:
        try:
            ...
            with open(f, "r", errors="replace") as fh:
                for line in fh:
                    ...
                    # Agent files only: first user message = the injected task description
                    if is_agent and role in ("user", "human") and not first_user_done:
                        if isinstance(content, list):
                            if all(isinstance(i, dict) and i.get("type") in ("tool_result", "tool_use")
                                   for i in content):
                                continue
    ...
    print(f"Total in-range JSONL events (for hours): {total_lines}", flush=True)
